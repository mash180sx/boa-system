...
(function() {
  ...
  */
  ...
  dbcb = function(err, client) {
    var Categories, Commodities, JANS, index, stream;
    if (err) {
      throw err;
    }
      ...
      if (doc != null) {
        ...
        if ((++index) % 10 === 0) {
          apac.getApaclist(conf.amazon, JANS, function(err, items) {
            var item, j, _i, _len, _results;
            _results = [];
            for (j = _i = 0, _len = items.length; _i < _len; j = ++_i) {
              item = items[j];
              _results.push(console.log(j, JSON.stringify(item)));
            }
            return _results;
          });
          return JANS = [];
